...
def my_parse_args():
    ...
    parser.add_argument('-r', '--reverse_edges_flag', action='store_true', default=False, help='reverse_edges_flag')#我的
...
def main():
    ...
    for dataname in config_sampling[args.scale]['datasets']:
        kg = load_kg(args.data_root,dataname,reverse_edges_flag=args.reverse_edges_flag)#我的
        num_ent = kg.num_ent
        num_rel = kg.num_rel
        ...
        #这里是根据每个spilt对每个pattern需采样多少次，生成重复的pattern_str的迭代器，放在patterns_pool里。比如train里每个pattern要采样1000次，就生成1000个这个pattern_str的迭代器。最后用chain.from_iterable把这些迭代器拼接成一个大的迭代器，供后续采样函数使用。
        ...
        def sample_mode(mode):
            ...
            if args.nproc == 1:
                ...
                for pattern_str in tqdm(patterns_pool[mode], total=patterns_total[mode]):
                    ...
                    #把采样的query和答案的dict放在answers_queries的list里
            else:
